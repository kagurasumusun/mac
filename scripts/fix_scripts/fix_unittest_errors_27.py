...
def write_stdout_bytes(data: bytes | str) -> None:
    if hasattr(sys.stdout, "buffer") and sys.stdout.buffer is not None:
        try:
            ...
            pass
    try:
        sys.stdout.write(data if isinstance(data, str) else data.decode("utf-8", errors="replace")) # type: ignore
        return
    except TypeError:
        pass
    sys.stdout.write(data.decode("utf-8", errors="replace") if isinstance(data, (bytes, bytearray)) else str(data)) # type: ignore
