...
for i in range(len(im_lines)):
    if "child_width = width // (scale or 1)" in im_lines[i] or "child_width = width // scale" in im_lines[i]:
        im_lines[i] = "    child_width = width // scale\n"
    if "child_height = height // (scale or 1)" in im_lines[i] or "child_height = height // scale" in im_lines[i]:
        im_lines[i] = "    child_height = height // scale\n"
    if "asset_scale = scale or 1" in im_lines[i] or "asset_scale = scale" in im_lines[i]:
...
for i in range(len(cp_lines)):
    if "result[\"TVTopShelfImage\"] = tv" in cp_lines[i]:
        cp_lines[i] = "                result[\"TVTopShelfImage\"] = tv # type: ignore\n"
