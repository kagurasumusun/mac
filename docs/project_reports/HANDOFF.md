...
- **History purge done (2026-07-17)**: `git filter-repo` removed `firefox-Assets.car`/`filemerge-Assets.car`/`timac-article.html` from ALL history; force-push `e969d3c → 9e8c364`; final tree verified diff-zero against the pre-purge tip; every referenced pre-`9e8c364` SHA in docs is superseded (content-identical). Fixture-suite v2 (vec/ios specials) generated via one-shot workflow; see ENGINEERING_LOG.
...
- `assetutil --info` semantic parity (Apple consumer as judge): basic 5/5, colordata 4/4, brand 14/14, scales 7/8, probe3a 21/23, probe3b 5/6, probe6 22/24 via tools/assetutil_semantic_matrix.py — the only residual differences are atlas bin-packing geometry (Apple's private MaxRects-style heuristic is not replicated; documented cosmetic).
- probe6 finalized the layout-12 dmp2 grammar rules: v4 palette for <=255-color sources above 512 raw bytes (ordinary images too, not only uniforms), v2 LZFSE below/above that for richer sources, GA grammar from the straight-gray constancy (v3 LZFSE frame / v2), MLEC mode 2 for all opaque non-CBCK sources; dmp2 stream-length is u32 everywhere (fixed latent >=64 KiB corruption). v3-mini is the only Apple form still undecoded (we emit valid substitutes).
- Remaining diff_cars residuals vs Apple oracles: identifier/localization 16-bit hash (deterministic pure-name function; positional weights 33^(k+3) mod 2^16 for k<=3 measured, final mixing at >2^31 crossings unidentified), v3-mini dmp2 grammar (partially decoded, mid-token rule open), LZFSE encoder quality, CBCK band-chunking heuristic, radiosity approximation — all catalogued in "Unfinished work".
- Remote Mac (session `wvfFmRNMM7L0THD7wXmF`) went unreachable mid-session on 2026-07-16 (uptermd auth rejection); probe6 (standalone gray-RGB(A) storage, GA v3 boundary refinement, two-color v4 usage) is staged locally in `tools/make_probe6.py` and awaits a reachable host.
- Head: `43cf850` pushed (probe6 grammar batch). Current session head after this batch: see `git log`. Push to `kagurasumusun/mac:actool` uses the Mac host (osxkeychain credentials) via the "Push procedure" below. Active upterm session: `wvfFmRNMM7L0THD7wXmF`.
...
Upterm session (2026-07-16, second host): `wvfFmRNMM7L0THD7wXmF`
...
ssh -i /home/user/.ssh/arena_upterm_ed25519 \
  wvfFmRNMM7L0THD7wXmF@uptermd.upterm.dev
```
...
- LZVN stream option for v2 GA sources (public codec; Apple picks it for the ga_vgrad case; we emit LZFSE).
- probe6-resolved items removed from this list: standalone gray-RGB(A) storage (CONFIRMED GA8), opaque-mode and v4-palette boundaries.
