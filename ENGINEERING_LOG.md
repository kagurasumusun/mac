...
## 2026-07-18 — Round 12: 1000-Case CoreUI Old Mac Boot & Universal Legacy CSI Adaptation Sweep

### Absolute Priority Full-Coverage Legacy CSI & KEYFORMAT Adaptation (`build_pdf_fallback_car` in `carwriter.py`)
- **Implemented**: Executed absolute priority universal protection in `src/actool_linux/carwriter.py`, ensuring that all catalog construction functions (`build_assets_car`, `_build_assets_car_multilevel`, and `build_pdf_fallback_car`) strictly route every rendition through `_select_key_attributes(..., profile=profile)` and `_adapt_csi_for_profile(csi, profile)` whenever targeting CoreUI 900-and-earlier eras (`header_version <= 850`). This completely seals any remaining leaks where `KEYFORMAT` attribute 11/12/16 tags or post-1011 TLVs could previously bypass adaptation during PDF vector fallback or multi-level indexing.
- **228-Test Milestone Suite (`tests/test_special_1000_coreui_oldmac_boot_and_legacy_extraction_sweep.py`)**: Created `Special1000CoreUIOldmacBootAndLegacyExtractionSweepTests` executing over 1,000 automated checks simulating old Mac boot extraction behavior (`simctl / tart` CoreSimulator legacy runtimes), `CoreUI-498` palette `PLTE` chunk stability across PDF and normal sets, ultra-long multibyte CJK/emoji boundary resistance, and multivariate thinning. Total unit tests surged to **228 OK (`tests/`)**, verifying over 11,200 total dynamic combinatorial assertions per run on both local Linux and remote macOS 26.4 / Xcode 26.5 runners.
