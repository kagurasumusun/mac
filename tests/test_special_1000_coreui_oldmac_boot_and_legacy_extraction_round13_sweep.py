...
class Special1000CoreUIOldmacBootAndLegacyExtractionRound13SweepTests(unittest.TestCase):
    def test_1000_coreui_oldmac_boot_and_simctl_virtual_legacy_extraction_sweep(self):
        ...
        for pname in legacy_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_oldmac_r13.car"
                assets = [
                    png_rendition(f"OldMacR13_{sc}x_{app}", PNG, f"om_{sc}x.png", scale=sc, appearance=app)
                ...
                pdf_car = build_pdf_fallback_car("OldMacPDFR13", PDF, PNG, PNG, coreui_profile=pname)
                pdf_path = Path(tmp) / f"{pname}_pdf_r13.car"
    ...
    def test_1000_coreui_legacy_palette_img_plte_chunk_and_keyformat_harmony_sweep(self):
        ...
        for pname in ["coreui-498", "coreui-700", "coreui-800", "coreui-850"]:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_pal_r13.car"
                assets = [png_rendition(f"PalOldMacR13_{i}", PNG, f"p{i}.png", scale=1) for i in range(5)]
                car_bytes = build_assets_car(assets, platform="iphoneos", coreui_profile=pname)
                car_path.write_bytes(car_bytes)
                info = inspect(car_path)
                self.assertEqual(info["car_header"]["core_ui_version"], PROFILES[pname].header_version)
    ...
    def test_1000_ultralong_emoji_multibyte_hash_stability_round13_sweep(self):
        """Sweep 250 checks evaluating deep polynomial hash stability across 250-byte CJK and emoji boundaries."""
        base_patterns = [
            "AppIcon-👨‍👩‍👧‍👦/Layer_01/EmojiFamilyTest_Round13",
            "Facet-日本語漢字テストと全角記号★☆♦/Scale_3x",
            "DeepHierarchy_Round13/" * 12 + "TerminalLeafName",
            "UltraLongAsciiName_" + "U" * 210,
        ]
    ...
    def test_1000_multivariate_thinning_combinatorial_scale_and_subtype_round13_sweep(self):
        """Sweep 200 combinatorial evaluations across multivariate thinning and repack boundaries."""
        assets = [
            png_rendition("ThinR13", PNG, "univ_1x.png", idiom="universal", scale=1),
            png_rendition("ThinR13", PNG, "phone_2x.png", idiom="iphone", scale=2),
            png_rendition("ThinR13", PNG, "phone_3x.png", idiom="iphone", scale=3),
            png_rendition("ThinR13", PNG, "pad_2x.png", idiom="ipad", scale=2),
            png_rendition("ThinR13", PNG, "watch_2x.png", idiom="watch", scale=2),
        ]
        options_list = [
