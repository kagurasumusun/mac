...
class Special1000CoreUILumeTartAndLegacyExtractionSweepTests(unittest.TestCase):
    def test_1000_coreui_lume_tart_virtual_runtime_legacy_extraction_sweep(self):
        """Sweep 300 checks simulating lume/tart virtual runtime extractions across CoreUI-498..850."""
        legacy_profiles = ["coreui-498", "coreui-700", "coreui-800", "coreui-850"]
        for pname in legacy_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_lume_check.car"
                assets = [
                    png_rendition(f"LumeIcon_{sc}x_{app}", PNG, f"lume_{sc}x.png", scale=sc, appearance=app)
    ...
    def test_1000_coreui_legacy_palette_img_plte_chunk_and_keyformat_sweep(self):
        """Sweep 250 checks across legacy palette-img formats under CoreUI-498/700 constraints without decode errors."""
        legacy_profiles = ["coreui-498", "coreui-700", "coreui-800", "coreui-850"]
        for pname in legacy_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_pal_lume.car"
                assets = [png_rendition(f"PalLume_{i}", PNG, f"p{i}.png", scale=1) for i in range(5)]
                car_bytes = build_assets_car(assets, platform="iphoneos", coreui_profile=pname)
                car_path.write_bytes(car_bytes)
                info = inspect(car_path)
                self.assertEqual(info["car_header"]["core_ui_version"], PROFILES[pname].header_version)
    ...
    def test_1000_ultralong_multibyte_and_emoji_path_modulo_sweep(self):
        ...
        base_patterns = [
            "AppIcon-👨‍👩‍👧‍👦/Layer_01/EmojiFamilyTest_LumeTart",
            "Facet-日本語漢字テストと全角記号★☆♦/Scale_3x",
            "DeepHierarchy_Lume/" * 12 + "TerminalLeafName",
    ...
    def test_1000_multivariate_thinning_combinatorial_scale_and_subtype_sweep(self):
        ...
        assets = [
            png_rendition("ThinLume", PNG, "univ_1x.png", idiom="universal", scale=1),
            png_rendition("ThinLume", PNG, "phone_2x.png", idiom="iphone", scale=2),
            png_rendition("ThinLume", PNG, "phone_3x.png", idiom="iphone", scale=3),
            png_rendition("ThinLume", PNG, "pad_2x.png", idiom="ipad", scale=2),
            png_rendition("ThinLume", PNG, "watch_2x.png", idiom="watch", scale=2),
        ]
        options_list = [
