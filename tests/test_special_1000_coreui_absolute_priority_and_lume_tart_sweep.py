...
class Special1000CoreUIAbsolutePriorityAndLumeTartSweepTests(unittest.TestCase):
    def test_1000_coreui_legacy_900_earlier_xcode_and_lume_tart_keyformat_sweep(self):
        """Sweep 300 checks verifying that legacy CoreUI <= 850 strictly assigns basic/iOS KEYFORMAT without unknown tokens."""
        legacy_profiles = ["coreui-498", "coreui-700", "coreui-800", "coreui-850"]
        for pname in legacy_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_keyfmt.car"
                assets = [
                    png_rendition(f"KeyFmt_{sc}x_{app}", PNG, f"k_{sc}x.png", scale=sc, appearance=app)
    ...
    def test_1000_coreui_absolute_priority_palette_and_deepmap_harmony_sweep(self):
        """Sweep 250 checks across legacy & modern CSI structures guaranteeing zero decode failures on palette and deepmap sets."""
        for pname in ["coreui-498", "coreui-700", "coreui-850", "coreui-975-device"]:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_pal_deep.car"
                assets = [png_rendition(f"PalDeep_{i}", PNG, f"pd_{i}.png", scale=1) for i in range(5)]
                car_bytes = build_assets_car(assets, platform="iphoneos", coreui_profile=pname)
                car_path.write_bytes(car_bytes)
                info = inspect(car_path)
                self.assertEqual(info["car_header"]["core_ui_version"], PROFILES[pname].header_version)
    ...
    def test_1000_ultralong_multibyte_and_emoji_path_modulo_sweep(self):
        ...
        base_patterns = [
            "AppIcon-👨‍👩‍👧‍👦/Layer_01/EmojiFamilyTest_PriorityLume",
            "Facet-日本語漢字テストと全角記号★☆♦/Scale_3x",
            "DeepHierarchy_Abs/" * 12 + "TerminalLeafName",
            "UltraLongAsciiName_" + "A" * 210,
        ]
    ...
    def test_1000_multivariate_thinning_combinatorial_scale_and_subtype_sweep(self):
        ...
        assets = [
            png_rendition("ThinAbs", PNG, "univ_1x.png", idiom="universal", scale=1),
            png_rendition("ThinAbs", PNG, "phone_2x.png", idiom="iphone", scale=2),
            png_rendition("ThinAbs", PNG, "phone_3x.png", idiom="iphone", scale=3),
            png_rendition("ThinAbs", PNG, "pad_2x.png", idiom="ipad", scale=2),
            png_rendition("ThinAbs", PNG, "watch_2x.png", idiom="watch", scale=2),
        ]
        options_list = [
