...
class Special1000CoreUITartAndLegacyParitySweepTests(unittest.TestCase):
    def test_1000_coreui_tart_virtual_runtime_header_and_storage_sweep(self):
        """Sweep 300 checks across simulated tart/lume virtual runtime eras guaranteeing exact storage v15..v17 alignment."""
        historical_profiles = [
        ...
        for pname in historical_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_tart.car"
                assets = [
                    png_rendition(f"TartIcon_{sc}x", PNG, f"icon_{sc}x.png", scale=sc)
                    for sc in [1, 2, 3]
                ]
    ...
    def test_1000_coreui_legacy_palette_img_plte_chunk_and_layout_sweep(self):
        ...
        for pname in legacy_profiles:
            with tempfile.TemporaryDirectory() as tmp:
                car_path = Path(tmp) / f"{pname}_pal.car"
                assets = [png_rendition(f"Pal_{i}", PNG, f"p{i}.png", scale=1) for i in range(5)]
                car_bytes = build_assets_car(assets, platform="iphoneos", coreui_profile=pname)
                car_path.write_bytes(car_bytes)
                info = inspect(car_path)
                self.assertEqual(info["car_header"]["core_ui_version"], PROFILES[pname].header_version)
    ...
    def test_1000_ultralong_multibyte_and_emoji_path_overflow_sweep(self):
        """Sweep 250 checks evaluating deep polynomial hash stability across 250-byte CJK and emoji boundaries."""
        base_patterns = [
            "AppIcon-👨‍👩‍👧‍👦/Layer_01/EmojiFamilyTest_Tart",
            "Facet-日本語漢字テストと全角記号★☆♦/Scale_3x",
            "DeepHierarchy_Tart/" * 12 + "TerminalLeafName",
            "UltraLongAsciiName_" + "T" * 210,
        ]
    ...
    def test_1000_multivariate_thinning_combinatorial_scale_and_subtype_sweep(self):
        ...
        assets = [
            png_rendition("ThinTart", PNG, "univ_1x.png", idiom="universal", scale=1),
            png_rendition("ThinTart", PNG, "phone_2x.png", idiom="iphone", scale=2),
            png_rendition("ThinTart", PNG, "phone_3x.png", idiom="iphone", scale=3),
            png_rendition("ThinTart", PNG, "pad_2x.png", idiom="ipad", scale=2),
            png_rendition("ThinTart", PNG, "watch_2x.png", idiom="watch", scale=2),
        ]
        options_list = [
